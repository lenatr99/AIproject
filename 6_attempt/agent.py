...
n_steps = 4         # number of steps for multi-step learning
...
class Agent():
    ...
    def act(self, state, eps=0.):
        ...
        """
        state = transform_state(state, mode='conv')
        # print(state.shape)
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        self.n_step_buffer = deque(maxlen=n_steps)  # New: n-step buffer
    ...
    def add(self, state, action, reward, next_state, done, error, action_dist, weight = None):
        ...
        if len(self.n_step_buffer) == self.n_step_buffer.maxlen:
            ...
            reward = sum([self.n_step_buffer[i][2]*(GAMMA**i) for i in range(self.n_step_buffer.maxlen)])
            next_state, done = self.n_step_buffer[-1][-2:]
