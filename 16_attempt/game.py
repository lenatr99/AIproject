...
class Game():
    ...
    def step(self, action):
        temp_board = np.copy(self.game_board)
        if action == 0:  # Move Up
            self.game_board = merge_up(self.game_board)
        elif action == 1:  # Move Down
            self.game_board = merge_down(self.game_board)
        elif action == 2:  # Move Left
            self.game_board = merge_left(self.game_board)
        elif action == 3:  # Move Right
            self.game_board = merge_right(self.game_board)
        else:
            return temp_board, 0, True, {"info": "invalid action"}

        done = False
        if np.array_equal(temp_board, self.game_board):  # if no movement was made, game over
            done = True
        else:
            self.add_random_tile()  # add a new tile at random position

        reward = np.sum(self.game_board)
        return self.game_board, reward, done, {"info": "action was taken"}
