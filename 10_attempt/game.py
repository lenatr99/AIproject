...
class Game():
    ...
    def reward_for_any_corner(self):
        """ Provides a reward if the maximum tile is in any corner and the tiles are ordered """
        max_tile = np.max(self.game_board)
        base_reward = 0
        board = self.game_board
        
        # Check if the max tile is in a corner
        corners = [board[0,0]]
        # print(max_tile in corners)
        if max_tile in corners:
            # Now we calculate the rewards for the tiles decreasing in value from left to right and top to bottom.
            for i in range(self.board_dim - 1):
                for j in range(self.board_dim - 1):
                    if board[i,j] >= board[i,j+1] and board[i,j] >= board[i+1,j]:
                        base_reward += 0.1*np.log2(max_tile)
    ...
    def calc_board(self, board):
        ...
        max_tile = np.max(shifted_board)  # Track the current maximum tile value
    ...
    def step(self, action, action_values):
        ...
        old_empty_cells = np.sum(self.game_board == 0)  # Add this line to count the empty cells before the action
        ...
        if not np.array_equal(self.game_board, temp_board):
            ...
            new_empty_cells = np.sum(self.game_board == 0)  # Add this line to count the empty cells after the action
            merges = new_empty_cells - old_empty_cells  # Calculate the number of merges
            # print(f"merges: {merges}")

            # Reward is the sum of the merged cells minus step cost plus merge count
            self.reward = self.reward - self.current_cell_move_penalty

            # adjust reward based on tile order
            self.reward += self.reward_for_any_corner()
